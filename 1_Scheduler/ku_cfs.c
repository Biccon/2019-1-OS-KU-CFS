...
void scheduler_handler(int signum)
{
    ...
    int nice_value = current_process->nice_value;
    current_process->vrumtime += 1 * calc_nice[nice_value + 2]; // 방금 멈춘 프로세스의 vrumtime을 계산하여 업데이트 한다.

    sort_by_vruntime(list); // vrumtime을 기준으로 list를 정렬한다.
    ...
    for (int i = 0; i < total_count; i++)
    {
        ...
        if (child > 0)
        {
            while (1)
            {
                ...
                {
                    insert_last(list, child, get_nice(nice_value, i), 0);
                    break;
                }
            }
        }
...
int main(int argc, char **argv)
{
    ...
    for (int k = 0; k < 5; k++)
    {
        nice_value[k] = atoi(argv[k + 1]);
        total_count += nice_value[k];
    }
    time_slice = atoi(argv[6]);

    list = (LinkedList *)malloc(sizeof(LinkedList));
    init_list(list);
    make_children(total_count, nice_value);
    set_timer();
}